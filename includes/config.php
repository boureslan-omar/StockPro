...
function requireRole(string ...$roles): void {
    if (!isLoggedIn()) {
        $redirect = urlencode($_SERVER['REQUEST_URI'] ?? '/stockpro/');
        header('Location: /stockpro/login.php?redirect=' . $redirect); exit;
    }
    ...
    if (!empty($roles) && !userCan(...$roles)) {
        ...
        <a href="/stockpro/" class="btn btn-primary">← Dashboard</a></div></body></html>';
        exit;
    }
}
