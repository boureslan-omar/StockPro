...
export default async function CashRegisterData({ from, to }: { from: string; to: string }) {
  ...
  const [{ usd: balanceUsd, lbp: balanceLbp }, { data: lastShift }, { data: shiftHistory }, { data: log }] = await Promise.all([
    getCashBalance(supabase),
    supabase.from("cash_shifts").select("closed_at").order("closed_at", { ascending: false }).limit(1).maybeSingle(),
    supabase.from("cash_shifts").select("*, profiles(full_name)").order("closed_at", { ascending: false }).limit(20),
    supabase
