...
// No hardcoded @page size — the browser's own print dialog is where the user
// picks their printer/paper size (a thermal printer, a regular one, or "Save
// as PDF" at any size). Relative widths so the same markup reflows cleanly
// whichever they choose, instead of us pre-deciding thermal vs A4 up front.
const RECEIPT_CSS = `
  @page { margin: 10mm; }
...
export function printReceiptWindow(
  ...
  win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Invoice ${esc(sale.receipt_no)}</title>
    <style>${RECEIPT_CSS}</style></head><body>${body}
