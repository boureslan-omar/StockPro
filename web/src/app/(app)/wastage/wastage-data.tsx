...
const REASON_KEYS = ["expired", "damaged", "owner_use", "sample", "lost", "other"];

export default async function WastageData({
  reason,
  from,
  to,
  prodFilter,
  ...
  const [{ data: records }, { data: mtdRows }, { data: expiringBatches }] = await Promise.all([
    query.order("wastage_date", { ascending: false }).order("id", { ascending: false }),
    supabase
      .from("wastage")
