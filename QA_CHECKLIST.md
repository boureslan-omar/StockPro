...
**Before starting:** Run `http://localhost/stockpro/upgrade_all.php` once to ensure all schema upgrades are applied.
...
|---|------|----------|-------|-------|
| 1-01 | Open `http://localhost/stockpro/` without logging in | Redirect to login page |pass | |
...
| 1-05 | As cashier, try to navigate directly to `/stockpro/pages/reports.php` | Access denied or redirect to login |pass | |
...
| # | Test | Expected | Grade | Notes |
|---|------|----------|-------|-------|
| 16-01 | With no license.lic present, open any system page (e.g. `/stockpro/pages/pos.php`) while logged in | Redirected to `/stockpro/pages/activate.php?reason=...` |pass | |
...
| 16-03 | On the developer machine open `http://localhost/stockpro/tools/keygen.php` → enter the Machine ID and a client name → Generate | A license key (long Base64 string) is generated | pass| |
...
| 16-10 | Try to access `http://localhost/stockpro/tools/keygen.php` from a device on the network (not localhost) | 403 Access Denied | pass| |
