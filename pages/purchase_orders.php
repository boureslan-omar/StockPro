    ...
    if ($po && !empty($pItems)) {
        ...
        try {
            ...
            foreach ($pItems as $pi) {
                ...
                $expiryDate  = trim($pi['expiry_date'] ?? '') ?: null;
                ...
                if ($pid && $prodSrc === 'consignment') {
                    ...
                    if ($itemUnit === 'box' && $boxUpb > 1) {
                        ...
                        if ($existing) {
                            ...
                            $pdo->prepare("INSERT INTO batches (product_id,purchase_id,cost_price,quantity_original,quantity_remaining,purchase_date,expiry_date) VALUES (?,?,?,?,?,CURDATE(),?)")
                                ->execute([$pid, $purchaseId, round($costPerUnit, 4), $totalUnits, $totalUnits, $expiryDate]);
                        ...
                        if ($existing) {
                            ...
                            $pdo->prepare("INSERT INTO batches (product_id,purchase_id,cost_price,quantity_original,quantity_remaining,purchase_date,expiry_date) VALUES (?,?,?,?,?,CURDATE(),?)")
                                ->execute([$pid, $purchaseId, $cost, $qty, $qty, $expiryDate]);
                            $batchId = (int)$pdo->lastInsertId(); $batchAction = 'new';
                        }
                        $pdo->prepare("INSERT INTO purchase_items (purchase_id,product_id,product_name,product_type,quantity,unit_cost,total,batch_id,batch_action) VALUES (?,?,?,'regular',?,?,?,?,?)")
                    ...
                    if ($itemUnit === 'box' && $newUpb > 1) {
                        ...
                        if ($newSrc !== 'consignment') {
                            $pdo->prepare("INSERT INTO batches (product_id,purchase_id,cost_price,quantity_original,quantity_remaining,purchase_date,expiry_date) VALUES (?,?,?,?,?,CURDATE(),?)")
                                ->execute([$newPid, $purchaseId, $costPerUnit, $totalUnits, $totalUnits, $expiryDate]);
                        ...
                        if ($newSrc !== 'consignment') {
                            $pdo->prepare("INSERT INTO batches (product_id,purchase_id,cost_price,quantity_original,quantity_remaining,purchase_date,expiry_date) VALUES (?,?,?,?,?,CURDATE(),?)")
                                ->execute([$newPid, $purchaseId, $cost, $qty, $qty, $expiryDate]);
                            $batchId = (int)$pdo->lastInsertId();
                            $pdo->prepare("INSERT INTO purchase_items (purchase_id,product_id,product_name,product_type,quantity,unit_cost,total,batch_id,batch_action) VALUES (?,?,?,'regular',?,?,?,?,'new')")
...
<div class="modal-body">
    ...
    <div class="position-relative mb-3">
        <div class="input-group">
            ...
            <input type="text" id="po-product-search" class="form-control"
                   placeholder="Search product by name or barcode to add…" autocomplete="off">
        </div>
        <div id="po-product-drop" class="list-group position-absolute w-100 shadow"
             style="z-index:9999;display:none;max-height:220px;overflow-y:auto"></div>
    </div>

    <!-- Checklist of selected items -->
    <div id="poChecklist"></div>
    <div id="poNoItems" class="text-center text-muted py-3 small">
        <i class="bi bi-search me-1"></i>Search and select products above to add them to the order.
    </div>
</div>
...
// ── PO product search ─────────────────────────────────────────────────────────
const poSearchInput = document.getElementById('po-product-search');
const poSearchDrop  = document.getElementById('po-product-drop');
let poSearchTimer = null;

if (poSearchInput) {
    poSearchInput.addEventListener('input', () => {
        clearTimeout(poSearchTimer);
        const q = poSearchInput.value.trim();
        if (q.length < 1) { poSearchDrop.style.display = 'none'; return; }
        poSearchTimer = setTimeout(() => {
            fetch(`/stockpro/pages/api.php?action=search_products_purchase&q=${encodeURIComponent(q)}`)
                ...
                .then(list => {
                    ...
                    if (!exact) items.push({id: null, name: q, unit:'pcs', cost_price:'', stock:0, _new:true});
                    poSearchDrop.innerHTML = items.map(p => `
                        <button type="button" class="list-group-item list-group-item-action py-2 px-3 small"
                                onclick='addPOItemFromSearch(${JSON.stringify(p)})'>
                            ${p._new ? '<span class="badge bg-success me-1">+ New Product</span>' : ''}
                            <strong>${p.name}</strong>
                            ${p.id ? `<span class="text-muted ms-2">${p.unit||''}</span>
                            <span class="float-end text-muted small">Stock: ${parseFloat(p.stock||0).toFixed(2)}</span>` : ''}
                        </button>`).join('');
                    poSearchDrop.style.display = 'block';
                });
        }, 250);
    });
    document.addEventListener('click', e => {
        if (!e.target.closest('#po-product-search') && !e.target.closest('#po-product-drop'))
            poSearchDrop.style.display = 'none';
    });
}

function addPOItemFromSearch(p) {
    poSearchDrop.style.display = 'none';
    poSearchInput.value = '';
    if (p.id && poSelectedIds[p.id] !== undefined) {
        const existing = document.getElementById(`po-item-${poSelectedIds[p.id]}`);
        existing?.scrollIntoView({block:'nearest', behavior:'smooth'});
        document.getElementById(`po-qty-${poSelectedIds[p.id]}`)?.focus();
        return;
    }
    const i = poRowIdx++;
    if (p.id) poSelectedIds[p.id] = i;
    const isNew = !p.id;
    document.getElementById('poNoItems').style.display = 'none';

    const div = document.createElement('div');
    div.className = 'list-group-item p-2 mb-2 rounded border';
    div.id = `po-item-${i}`;
    div.innerHTML = `
        <div class="d-flex align-items-start gap-2">
            <div class="flex-grow-1">
                <div class="d-flex align-items-center gap-2 mb-2">
                    ...
                    ${isNew ? '<span class="badge bg-info text-dark" style="font-size:.65rem">NEW</span>' : ''}
                    ${p.stock != null ? `<span class="ms-auto text-muted small">In stock: ${parseFloat(p.stock).toFixed(2)} ${p.unit||''}</span>` : ''}
                </div>
                <input type="hidden" name="items[${i}][product_id]"   value="${p.id||''}">
                <input type="hidden" name="items[${i}][product_name]" value="${p.name.replace(/"/g,'&quot;')}">
                <div class="row g-2">
                    <div class="col-sm-3">
                        <label class="form-label mb-0" style="font-size:.7rem">Qty *</label>
                        <input type="number" name="items[${i}][quantity]" id="po-qty-${i}"
                               class="form-control form-control-sm" min="0.001" step="0.001" required placeholder="0">
                    </div>
                    <div class="col-sm-2">
                        <label class="form-label mb-0" style="font-size:.7rem">Unit</label>
                        <select name="items[${i}][unit]" id="po-unit-${i}" class="form-select form-select-sm" onchange="poToggleBoxOpts(${i},${p.id?'false':'true'})">
                            ${['pcs','kg','box','crate','doz','L','pack'].map(u=>`<option${u===(p.unit||'pcs')?' selected':''}>${u}</option>`).join('')}
                        </select>
                    </div>
                    <div class="col-sm-4">
                        <label class="form-label mb-0" style="font-size:.7rem">Est. Price</label>
                        <div class="input-group input-group-sm">
                            <input type="number" name="items[${i}][estimated_price]" id="po-eprice-${i}"
                                   class="form-control form-control-sm" value="${p.cost_price||''}"
                                   min="0" step="0.0001" data-cur="usd" oninput="poUpdateHintNew(${i})">
                            <button type="button" id="po-eu-${i}" class="btn btn-outline-secondary px-1"
                                    style="font-size:.6rem;min-width:22px;font-weight:bold" onclick="poToggleCurNew(${i},'usd')">$</button>
                            <button type="button" id="po-el-${i}" class="btn btn-outline-secondary px-1 opacity-50"
                                    style="font-size:.6rem;min-width:22px" onclick="poToggleCurNew(${i},'lbp')">L£</button>
                        </div>
                        <div id="po-ehint-${i}" class="text-muted" style="font-size:.6rem"></div>
                    </div>
                    <div class="col-sm-3">
                        <label class="form-label mb-0" style="font-size:.7rem">Note</label>
                        <input type="text" name="items[${i}][note]" class="form-control form-control-sm" placeholder="Optional">
                    </div>
                </div>
                ${isNew ? `
                <div class="mt-2">
                    <label class="form-label mb-1" style="font-size:.7rem">New Product Type</label>
                    <div class="btn-group btn-group-sm w-100" role="group">
                        <input type="radio" class="btn-check" name="items[${i}][new_product_source]" id="po-sreg-${i}" value="regular" checked>
                        <label class="btn btn-outline-primary btn-sm" for="po-sreg-${i}" style="font-size:.75rem">Regular</label>
                        <input type="radio" class="btn-check" name="items[${i}][new_product_source]" id="po-scon-${i}" value="consignment">
                        <label class="btn btn-outline-warning btn-sm" for="po-scon-${i}" style="font-size:.75rem">Consignment</label>
                    </div>
                    <div id="po-boxopts-${i}" class="d-none mt-2">
                        <div class="input-group input-group-sm">
                            <span class="input-group-text" style="font-size:.7rem">📦 Units/Box</span>
                            <input type="number" name="items[${i}][new_product_upb]" id="po-upb-${i}"
                                   class="form-control form-control-sm" value="1" min="1" step="1">
                        </div>
                    </div>
                </div>` : `<input type="hidden" name="items[${i}][new_product_source]" value="regular">
                <input type="hidden" name="items[${i}][new_product_upb]" value="1">`}
            </div>
            <button type="button" class="btn btn-sm btn-outline-danger mt-1 flex-shrink-0"
                    onclick="removePOItem(${i},${p.id||'null'})" title="Remove">
                <i class="bi bi-x-lg"></i>
            </button>
        </div>`;
    document.getElementById('poChecklist').appendChild(div);
    setTimeout(() => document.getElementById(`po-qty-${i}`)?.focus(), 50);
}

function removePOItem(i, pid) {
    document.getElementById(`po-item-${i}`)?.remove();
    if (pid) delete poSelectedIds[pid];
    if (!document.getElementById('poChecklist').children.length)
        document.getElementById('poNoItems').style.display = '';
}

function poToggleBoxOpts(i, isNew) {
    if (!isNew) return;
    const unit  = document.getElementById(`po-unit-${i}`)?.value;
    const boxEl = document.getElementById(`po-boxopts-${i}`);
    if (boxEl) boxEl.classList.toggle('d-none', unit !== 'box');
}

function poToggleCurNew(i, newCur) {
    const inp = document.getElementById('po-eprice-'+i);
    if (!inp) return;
    ...
    document.getElementById(`po-eu-${i}`)?.classList.toggle('opacity-50', newCur !== 'usd');
    document.getElementById(`po-el-${i}`)?.classList.toggle('opacity-50', newCur !== 'lbp');
    poUpdateHintNew(i);
}

function poUpdateHintNew(i) {
    const inp  = document.getElementById('po-eprice-'+i);
    const hint = document.getElementById('po-ehint-'+i);
    if (!inp || !hint) return;
    const val = parseFloat(inp.value) || 0;
    const cur = inp.dataset.cur || 'usd';
    hint.textContent = !val ? '' : cur === 'lbp'
        ? '≈ $' + (val/PO_RATE).toFixed(2)
        : '≈ L£ ' + Math.round(val*PO_RATE).toLocaleString();
}

// Convert any LBP prices to USD before submitting
document.getElementById('poForm')?.addEventListener('submit', function() {
    this.querySelectorAll('[id^="po-eprice-"]').forEach(inp => {
...
function sendWhatsApp(poId, phone, poNumber) {
    window.open('/stockpro/pages/po_view.php?id=' + poId + '&autoprint=1', '_blank');
    setTimeout(function() {
...
function openReceive(poId, poNumber, supplierId) {
    fetch('/stockpro/pages/api.php?action=po_items&po_id=' + poId)
        .then(r => r.json())
        .then(items => {
            ...
            items.forEach((it, i) => {
                ...
                tr.innerHTML = `
                    <td>
                        ...
                        ${parseInt(it.track_expiry) ? `<div class="mt-2">
                            <label class="form-label small fw-semibold text-danger mb-1"><i class="bi bi-calendar-x me-1"></i>Expiry Date *</label>
                            <input type="date" name="pitems[${i}][expiry_date]" class="form-control form-control-sm" required>
                        </div>` : '<input type="hidden" name="pitems[' + i + '][expiry_date]" value="">'}
                    </td>
