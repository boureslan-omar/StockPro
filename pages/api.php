...
if ($action === 'po_items') {
    ...
    $stmt = $pdo->prepare("
        SELECT poi.id, poi.po_id, poi.product_id, poi.product_name,
               ...
               COALESCE(p.track_expiry, 0) AS track_expiry
...
if ($action === 'get_customer_prices') {
    $custId = (int)($_GET['customer_id'] ?? 0);
    if (!$custId) { echo json_encode([]); exit; }
    $stmt = $pdo->prepare("SELECT product_id, last_price FROM customer_prices WHERE customer_id = ?");
    $stmt->execute([$custId]);
    $prices = [];
    foreach ($stmt->fetchAll() as $row) {
        $prices[(int)$row['product_id']] = (float)$row['last_price'];
    }
