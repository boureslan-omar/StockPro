...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($cartData)) {
        ...
        try {
            ...
            // Upsert customer-specific prices for each item sold
            if ($customerId) {
                $upsertPrice = $pdo->prepare("
                    INSERT INTO customer_prices (customer_id, product_id, last_price)
                    VALUES (?, ?, ?)
                    ON DUPLICATE KEY UPDATE last_price = VALUES(last_price), updated_at = CURRENT_TIMESTAMP
                ");
                foreach ($cartData as $item) {
                    $pid = (int)($item['product_id'] ?? 0);
                    $itemPrice = (float)($item['price'] ?? 0);
                    if ($pid > 0 && $itemPrice > 0 && ($item['type'] ?? '') !== 'bulk') {
                        $upsertPrice->execute([$customerId, $pid, $itemPrice]);
                    }
                }
            }
...
<div class="modal-body receipt p-4">
    ...
    <div class="text-center mb-3">
        <?php if (file_exists($_SERVER['DOCUMENT_ROOT'] . '/stockpro/assets/img/logo.png')): ?>
        <img src="/stockpro/assets/img/logo.png" alt="logo" style="height:72px;width:72px;object-fit:contain;margin-bottom:6px">
        <br>
        <?php endif; ?>
...
<div class="modal-footer no-print">
    ...
    <a href="/stockpro/pages/pos.php" class="btn btn-primary">New Sale</a>
</div>
</div>
</div>
</div>
...
if (barcodeInput) {
    barcodeInput.addEventListener('keydown', e => {
        ...
        if (e.key === 'Escape') {
            barcodeInput.value = '';
            applyCategory(document.querySelector('.cat-btn.active')?.dataset?.cat || 'all');
        }
    });
...
function triggerSearch() {
    ...
    if (!q) return;
    fetch(`/stockpro/pages/api.php?action=search_product&q=${encodeURIComponent(q)}`)
        .then(r => r.json())
        .then(d => {
...
function addToCart(id, name, price, cost, stock, type = 'regular', forceQty = null) {
    id = String(id);
    // Apply customer-specific price if available and it's the first time adding
    if (type !== 'bulk' && !cart[id] && customerPrices[parseInt(id)] !== undefined) {
        price = customerPrices[parseInt(id)];
    }
...
function renderCart() {
    ...
    Object.entries(cart).forEach(([id, item]) => {
        ...
        const marginVal   = item.cost > 0 ? ((item.price / item.cost - 1) * 100).toFixed(1) : '';
        const marginColor = item.cost > 0
            ? (item.price >= item.cost * 1.05 ? '#22c55e' : item.price >= item.cost ? '#f59e0b' : '#dc3545')
            : '#adb5bd';
        const custBadge   = (customerPrices[parseInt(item.pid || id)] !== undefined)
            ? '<span title="Customer price" style="font-size:.5rem;background:#0d6efd;color:#fff;border-radius:2px;padding:0 3px;margin-left:2px">CP</span>' : '';
        html += `<tr>
            <td class="small">${item.name}${badge}${custBadge}</td>
            <td style="width:88px">
                <div class="input-group input-group-sm">
            ...
            <td style="width:${iCur==='lbp'?'120':'92'}px">
                ...
                ${item.cost > 0 ? `<div class="input-group input-group-sm mt-1" style="width:72px">
                    <input type="number" class="form-control form-control-sm text-end px-1" id="margin-inp-${id}"
                           value="${marginVal}" step="0.1" title="Margin %"
                           onchange="setItemMargin('${id}',this.value)"
                           style="width:46px;padding:2px;font-size:.65rem">
                    <span class="input-group-text px-1" style="font-size:.65rem;color:${marginColor};font-weight:bold;min-width:20px">%</span>
                </div>` : ''}
            </td>
...
function setItemPrice(id, val) {
    ...
    // Sync margin input
    const mInp = document.getElementById('margin-inp-' + id);
    if (mInp && cart[id].cost > 0) {
        mInp.value = ((cart[id].price / cart[id].cost - 1) * 100).toFixed(1);
    }
    let subtotal = 0;
...
function setItemMargin(id, pct) {
    pct = parseFloat(pct);
    if (isNaN(pct) || !cart[id]) return;
    const cost = cart[id].cost;
    if (cost <= 0) { showToast('Cost price not set for this product', 'warning'); return; }
    cart[id].price = parseFloat((cost * (1 + pct / 100)).toFixed(4));
    // Sync price input
    const priceInp = document.getElementById('price-inp-' + id);
    const cur = cartCur[id] || 'usd';
    if (priceInp) priceInp.value = cur === 'lbp'
        ? Math.round(cart[id].price * EXCHANGE_RATE)
        : cart[id].price.toFixed(2);
    const line = parseFloat((cart[id].qty * cart[id].price).toFixed(2));
    const el = document.getElementById('ln-' + id);
    if (el) el.textContent = formatUSD(line);
    let subtotal = 0;
...
function loadCustomerPrices(custId) {
    Object.keys(customerPrices).forEach(k => delete customerPrices[k]);
    fetch(`/stockpro/pages/api.php?action=get_customer_prices&customer_id=${custId}`)
        .then(r => r.json())
        .then(prices => {
            Object.assign(customerPrices, prices);
            // Apply prices to any items already in the cart
            let updated = false;
            Object.entries(cart).forEach(([id, item]) => {
                if (item.type === 'bulk') return;
                const pid = parseInt(item.pid || id.replace('_b',''));
                if (customerPrices[pid] !== undefined) {
                    item.price = customerPrices[pid];
                    updated = true;
                }
            });
            if (updated) { renderCart(); showToast('Customer prices applied to cart', 'info'); }
        })
        .catch(() => {});
}
...
if (custSearch) {
    custSearch.addEventListener('input', () => {
        ...
        fetch(`/stockpro/pages/api.php?action=search_customer&q=${encodeURIComponent(q)}`)
            .then(r => r.json())
            .then(list => {
...
function toggleAutoPrint() {
    ...
    fetch(`/stockpro/pages/api.php?action=set_setting&key=auto_print_receipt&val=${newVal}`)
