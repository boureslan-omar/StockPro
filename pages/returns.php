...
function fetchReceiptByNo(receiptNo) {
    fetch('/stockpro/pages/api.php?action=search_customer&q=' + encodeURIComponent(receiptNo))
        .then(r => r.json())
        .then(() => {
...
function loadCustomerSales(customer) {
    ...
    fetch('/stockpro/pages/api.php?action=search_customer&q=' + encodeURIComponent(customer.name))
        .then(() => {
    ...
    fetch('/stockpro/pages/api.php?action=customer_sales&customer_id=' + encodeURIComponent(customer.id))
        .then(r => r.json())
        .then(sales => {
...
function loadSaleItems(saleId, label) {
    ...
    const url = saleId
        ? '/stockpro/pages/api.php?action=sale_items_for_return&sale_id=' + saleId
        : '/stockpro/pages/api.php?action=sale_items_for_return_by_receipt&receipt_no=' + encodeURIComponent(label);
...
function submitCustReturn() {
    ...
    fetch('/stockpro/pages/api.php?action=process_customer_return', { method:'POST', body })
        .then(r => r.json())
        .then(d => {
...
function searchBatches() {
    ...
    el.innerHTML = '<div class="text-muted small py-2">Searching…</div>';
    fetch('/stockpro/pages/api.php?action=batches_for_supplier_return&q=' + encodeURIComponent(q))
        .then(r => r.json())
        .then(batches => {
...
function submitSupplierReturn() {
    ...
    fetch('/stockpro/pages/api.php?action=process_supplier_return', { method:'POST', body })
        .then(r => r.json())
        .then(d => {
